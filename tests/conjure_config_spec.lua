...
t.stub_module("conjure.client.clojure.nrepl.action", {
...
t.patch(vim, "defer_fn", function(callback, delay)
  deferred_callback = callback
  deferred_delay = delay
end)

t.patch(vim.fs, "root", function(bufnr, markers)
...
t.run(function()
  local spec = t.load("lua/sandu/plugins/neovim/conjure.lua")
  ...
  assert(
    type(deferred_delay) == "number" and deferred_delay > 0,
    "expected deferred REPL connect to use a positive delay"
  )
  ...
  assert(
    type(last_connect_opts) == "table",
    "expected deferred REPL connect callback to pass options to connect-port-file()"
  )
  assert(
    last_connect_opts["silent?"] == true,
    "expected deferred REPL connect callback to avoid Conjure auto-REPL fallback"
  )
